...
namespace AuthorizeNet
{
	...
	public class HttpXmlUtility
	{
		...
		private string _serviceUrl = "https://apitest.authorize.net/xml/v1/request.api";
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="T:AuthorizeNet.HttpXmlUtility" /> class.
		/// </summary>
		...
		public HttpXmlUtility(ServiceMode mode, string apiLogin, string transactionKey)
		{
			...
			{
				this._serviceUrl = "https://api.authorize.net/xml/v1/request.api";
			}
		...
		private void CheckForErrors(ANetApiResponse response)
		{
			...
			{
				createCustomerProfileTransactionResponse thingy = (createCustomerProfileTransactionResponse)response;
				thingy.directResponse = this._xmlDoc.ChildNodes[1].ChildNodes[1].InnerText;
				response = thingy;
				return;
			}
			if ((int)response.messages.message.Length > 0 && response.messages.resultCode == messageTypeEnum.Error)
			{
				StringBuilder sb = new StringBuilder();
				for (int i = 0; i < (int)response.messages.message.Length; i++)
				{
		...
		private ANetApiResponse DecideResponse(XmlDocument xmldoc)
		{
			...
			try
			{
				StringReader reader = new StringReader(xmldoc.DocumentElement.OuterXml);
			...
			catch (Exception exception)
			{
				...
				Console.WriteLine(string.Concat(ex.GetType().ToString(), ": ", ex.Message));
			}
			return apiResponse;
		}
		...
		{
			this.AuthenticateRequest(apiRequest);
			HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create(this._serviceUrl);
			...
			this._xmlDoc.Load(XmlReader.Create(webResponse.GetResponseStream()));
			ANetApiResponse response = this.DecideResponse(this._xmlDoc);
